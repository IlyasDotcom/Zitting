export default function BijsturenOnderDruk() {
  const moments = [
    {
      label: 'Moment 1 — week 6–7', title: 'Scope-druk', color: 'var(--purple-base)',
    ...
    {
      label: 'Moment 2 — week 11', title: 'Code review-druk', color: 'var(--petrol-base)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%' }}>
      {moments.map((m, i) => (
        <div key={i} style={{ padding: '18px 20px', borderRadius: 'var(--radius-md)', background: 'var(--surface-card)', border: '1px solid var(--border)', borderLeft: `4px solid ${m.color}`, boxShadow: 'var(--shadow-sm)', animation: 'cardIn 0.3s both', animationDelay: `${i * 0.1}s` }}>
          <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.6px', textTransform: 'uppercase', color: m.color, marginBottom: '4px' }}>{m.label}</div>
          <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--ink)', marginBottom: '12px' }}>{m.title}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            {[{ key: 'Situatie', val: m.situation }, { key: 'Aanpak', val: m.action }, { key: 'Effect', val: m.effect }].map((col, j) => (
      ...
      <div style={{ padding: '14px 20px', borderRadius: 'var(--radius-md)', background: 'var(--petrol-lighter)', border: '1px solid var(--border)', fontSize: '13px', fontWeight: 600, color: 'var(--petrol-darker)', textAlign: 'center', animation: 'cardIn 0.3s both', animationDelay: '0.2s' }}>
